...
int main(){
	ios::sync_with_stdio(0), cin.tie(0);
		...
		/*/
		freopen("../python/hippo_color_rgb.txt", "r", stdin);
		freopen("../javascript/MazeShapes/HippoShape.js", "w", stdout);
		//*/
	#endif
	...
	for(int i=0; i<height; ++i){
		for(int j=0; j<width; ++j){
			...
			int bound[3] = {255, 255, 255};			
			images[i][j] = !(abs(b - bound[0]) <= 20  && abs(g - bound[1]) <= 20 && abs(r - bound[2]) <= 20);
			if(images[i][j]){
				left = min(left, j);
				right = max(right, j);
				top = min(top, i);
